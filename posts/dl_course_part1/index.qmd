...
A few months ago, when looking for a course to consolidate and extend my knowledge of Deep Learning, I came across various options: Ian Goodfellow's book, Jeremy Howard's fastai course, Andrew Ng's Deep Learning Specialization course - the list goes on and on. What I didn't come across was different people's thoughts for where best to start (for such a mature, important field, the tuition options are scattered). In this 2-part series of blogposts, I aim to chip away at that and provide my views on the course I ended up going for: Andrew Ng's Deep Learning Specialization. 

** Motivation **

I will not bore the reader with reasons why Deep Learning is a field worthy of one's attention. All you need to do is open the news to see the effects large neural networks (most recently, language models) are having on society in multiple ways. Rather, I will dial in on what some of my personal motivations were prior to taking this course: 
...
[GO OVER THE BASICS FROM THE LIST BELOW. INCLUDE DISCUSSIONS ON ORTHOGONALIZATION HERE]

* Intuition behind Neural Networks: why they work, how they differ from stats/ML frameworks
* High-level flow for creating a NN: create an architecture, initialise parameters, train the network (look over forward prop and backward prop + gradient descent), and most importantly of all... ITERATE
* The simple mathematical operations of single nodes: linear combination and "activation"
